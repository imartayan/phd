...
:::{.callout-caution}
Unedited draft from {{< longcite brisk >}}.
:::

## Introduction {#sec-intro-skmer-map}

Handling sequencing and genomic data is among the most memory-intensive tasks routinely performed in computational biology [@big-data-genomical; @cremin2022big].
...
Since the advent of BLAST, a fundamental need has emerged to index /kmers and associate information with them [@jenike2024guide; @survey-colored-kmer-sets; @survey-kmer-sets].
...
A viable solution to mitigate this memory usage is to avoid indexing all /kmers, instead using minimizers to ensure the scalability of tools [@minimap2; @metamdbg; @fhs; @niqki].
Recently, indexes capable of associating /kmers with additional information have been proposed.
These indexes generally fall into two categories: full-text indexes based on the Burrows-Wheeler Transform (BWT) (e.g., BOSS [@bowe2012succinct], Themisto [@themisto]) or Minimal Perfect Hash Functions (MPHF) (e.g., BBHash [@bbhash], PTHash [@pthash], Pufferfish [@pufferfish], Blight [@blight], SShash [@sshash]).
...
Therefore, they depend on such spectrum-preserving string sets (SPSS) [@spss] structures to construct their index, meaning that /kmers must be "assembled" before the indexing step.
...
Some semi-dynamic approaches have been proposed, using temporary buffers and partitions to delay reconstruction as much as possible [@alanko2021buffering; @dynamic-mantis].
...
However, most SPSS representations such as simplitigs [@spss; @simplitigs], matchtigs [@matchtigs], eulertigs [@eulertigs], and masked superstrings [@masked-superstring-kmer] are defined globally from a complete /kmer set.
Updating such SPSS structures remains an open problem, making their use in dynamic structures even more challenging [@cdbgtricks].
...
To complicate matters, buckets are known to have skewed size distributions, as lower minimizers tend to gather more /kmers than others leading to expensive search for a large number of /kmers [@bcalm1; @kmc2; @marchet2023scalable].
...
Additionally, some studies disregard the reverse-complement aspect of DNA sequences, which can have a significant impact in practice [@knonical].
