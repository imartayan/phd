# Super‑*k*‑mer maps {#sec-skmer-maps}
...
Super-/kmers were presented there primarily as a partitioning and streaming tool.
This chapter asks a different question: can /skmers serve as the primary representation inside a dynamic /kmer dictionary, replacing individual /kmer entries altogether?

Two problems arise that do not appear when /skmers are used merely for partitioning.
...
Static /skmer-based indexes such as SSHash [@sshash] take a different approach, relying on a precomputed spectrum-preserving string set and a minimal perfect hash function.
The structure described here is dynamic: /kmers can be inserted one at a time in streaming, without any prior knowledge of the full dataset.

## Canonical encoding of /skmers {#sec-encoding-skmer-map}

### Lazy encoding

Since all /skmers in a bucket share the same minimizer, that minimizer need not be stored in each /skmer individually.
...
This *lazy encoding* lowers the per-/kmer bit cost: a maximal /skmer of length $2k - m$ bases encodes on average $(k - m + 1)/2$ /kmers, giving a ratio of $\frac{8(k - m)}{k - m + 1}$ bits per /kmer, which for practical values of $k$ and $m$ is close to 8 bits.
...
The mean /skmer length grows with $k$ and shrinks with $m$, and can be further improved by using lower-density minimizer schemes as discussed in @sec-low-density-minimizers.

### Canonical minimizer selection

Lazy encoding requires that each /kmer be associated with a unique minimizer position and strand.
For most /kmers this is unambiguous: the minimal /mmer occurs once, and choosing the canonical strand fixes the encoding entirely.
When the canonical minimizer appears more than once within a /kmer or across its reverse complement, additional rules are needed to ensure determinism.

We handle this with a three-level selection procedure.
First, only canonical /mmers (those lexicographically smaller than their reverse complement) are considered as minimizer candidates, which immediately fixes the strand in the common case since only one strand contains the canonical /mmer.
Second, when the canonical /mmer occurs multiple times, the leftmost occurrence across both the /kmer and its reverse complement is selected.
Third, when two leftmost occurrences appear at the same relative position on different strands (the "mirror" case), the occurrence on the canonical /kmer strand is preferred.

@fig-brisk-canon illustrates all four cases that arise in practice.
This procedure is complete: every /kmer is assigned a unique minimizer occurrence regardless of $k$ parity, palindromic /kmers, or repeated /mmers.

![Four cases of canonical minimizer selection within a /kmer. Blue bars represent the canonical /kmer and gray bars the complementary /kmer. White arrows mark canonical /mmer occurrences and black arrows mark non-canonical ones. The selected occurrence is highlighted in green. Case A is the common case with no duplication. Cases B and C involve canonical occurrences on the same strand and on both strands respectively, resolved by rule 2. Case D is the mirror case where both strands yield a leftmost occurrence at the same position, resolved by rule 3 in favor of the canonical /kmer.](../fig/brisk/CANON){#fig-brisk-canon}

## Interleaved /skmers {#sec-interleaved-skmer-map}

### The bucket lookup problem

Once /skmers are grouped by minimizer into buckets, looking up a given /kmer requires finding whether it is contained in any of the bucket's /skmers.
...
The difficulty is that a bucket can hold many /skmers, and a linear scan is $\mathcal{O}(S)$ in the bucket size $S$.

The natural remedy is to sort the /skmers and use binary search.
However, sorting /skmers in standard lexicographic order puts the most weight on the outermost bases of the /skmer, which are precisely the least shared.
The leftmost base of a maximal /skmer belongs to only one /kmer in the /skmer, the second leftmost to two, and so on outward from the minimizer.
A sort order that prioritizes the outermost bases therefore fails to reduce the search space quickly: most /kmer queries share the discriminating bases only near the center, not at the edges.
...
With this property in place, /skmers can be sorted lexicographically in their interleaved form and queried by binary search.
...
Genomic sequences are not random strings: biological repeats, low-complexity regions, and reverse-complement symmetry all cause certain minimizers to attract far more /skmers than others, and a handful of very large buckets can dominate overall query time even when the average size is moderate.
