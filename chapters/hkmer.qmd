...
On one thousand *S. enterica* complete genomes from NCBI (@fig-plot-salmo), KFC remains the most memory-efficient tool, ahead of FastK and KMC.
For very large /kmer sizes ($k > 500$) KFC also becomes the fastest, although its multithreading efficiency (analyzed in @sec-kfc-multithread) lags behind KMC on small input sizes, so it leads only when running on fewer threads.
